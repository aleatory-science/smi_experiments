...
class ExpLogger:
    ...
    def __init__(self, experiment_name):
        ...
        logs_dir = _setup_dir(Path(__file__).parent.parent, "logs")
    ...
    def write_artifact(
        self, model_type, method, kernel, dataset, guide, model, params, hyper_params
    ):
    ...
    def load_logs(self, timestamp):
        assert any(
            timestamp in str(loc) for loc in self.base_dir.glob("*")
        ), f"Unknown log {timestamp}"
